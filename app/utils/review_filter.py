...
class ReviewFilter:
    """Service to filter informative reviews using AR-Miner model."""
    
    def __init__(self, model_path: str = None):
        """Initialize the review filter with model path."""
        self.model_path = model_path or os.path.join("app", "models", "ar_miner_model.pkl")
        self.model = None
        
    def load_model(self):
        """Load the AR-Miner model."""
        try:
            self.model = ARMiner.load_model(self.model_path)
            return True
        except Exception as e:
            logger.error(f"Failed to load AR-Miner model: {str(e)}")
            return False
            
    def filter_informative_reviews(self, reviews_df: pd.DataFrame, 
                                  status_logger: Optional[StatusLogger] = None) -> pd.DataFrame:
        """
        Filter reviews to keep only informative ones using the AR-Miner model.
        
        Args:
            reviews_df: DataFrame containing app reviews
            status_logger: Optional logger to track progress
            
        Returns:
            DataFrame containing only informative reviews with added informative flag and probability
        """
        log = status_logger or logger
        
        if reviews_df.empty:
            log.warning("No reviews to filter.")
            return reviews_df
        
        try:
            # Load model if not loaded
            if self.model is None:
                log.info("Loading AR-Miner model...")
                if not self.load_model():
                    return reviews_df
            
            log.info(f"Filtering {len(reviews_df)} reviews for informative content...")
            
            # Assuming the review content column is named 'content'
            # Adjust if your DataFrame uses a different column name
            is_informative, probabilities = self.model.filter_reviews(reviews_df['content'])
            
            # Add results to the dataframe
            reviews_df['informative'] = is_informative
            reviews_df['informative_prob'] = probabilities
            
            # Filter for only informative reviews
            informative_reviews = reviews_df[reviews_df['informative']].copy()
            
            log.info(f"Found {len(informative_reviews)} informative reviews out of {len(reviews_df)} total.")
            log.info(f"Informative reviews percentage: {len(informative_reviews) / len(reviews_df) * 100:.2f}%")
            
            return informative_reviews
            
        except Exception as e:
            log.error(f"Error filtering informative reviews: {str(e)}", exc_info=True)
            return reviews_df  # Return original dataframe in case of error